...
- *Amazon Athena*: Cho phép truy vấn SQL trên historical logs.
- *Amazon SNS*: Gửi alerts và notifications cho anomalies.
...
1. *Architecture Design*: Xác định serverless log pipeline và tích hợp dịch vụ AWS (Tháng 0).
2. *Cost Estimation & Optimization*: Sử dụng AWS Pricing Calculator để dự báo chi phí và điều chỉnh thiết kế (Tháng 1).
3. *System Setup & Integration*: Cấu hình CloudWatch Agents, Lambda functions, và Glue jobs (Tháng 2).
4. *Testing & Deployment*: Xác thực ingestion, classification, và alerting workflows; triển khai production (Tháng 3).
...
- *Trước thực tập (Tháng 0)*: 1 tháng cho việc lập kế hoạch và xem xét phương pháp thu thập logs hiện tại.
- *Thực tập (Tháng 1–3)*:
    - Tháng 1: Nghiên cứu dịch vụ AWS và cấu hình CloudWatch Agents cho applications.
        - Tháng 2: Thiết kế và điều chỉnh pipeline architecture, tích hợp Lambda, SQS, DynamoDB, và S3.
        - Tháng 3: Triển khai, kiểm thử, và khởi chạy hệ thống đầy đủ với Glue jobs và Athena queries.
